...
// Sync lane states when audio/data island is active
...
// Non-sync lane states when audio/data island is active
...
// Blank line region settings (top/bottom lines rendered as black)
...
// Larger chunk counts needed for audio/data island operation
...
// Use the larger size so the struct is always sufficient for audio mode
...
// Audio/data island variants -- insert HDMI data packets into blanking intervals
...
// Update the scanline DMA read address to point at a freshly-encoded TMDS buffer.
// audio=true uses the offset for with-audio DMA lists (more blocks per lane).
